...
async def account_user(page: ft.Page):
    class Container:
        def __init__(self):
            self.photo = ft.Image(
        ...
        async def verify_email_button(self, event):
            ...
            url = 'http://127.0.0.1/user/user/verifi_email'
            payload = {
                "email": email,
            }
        ...
        async def updates_account(self, event):
            ...
            url = 'http://127.0.0.1/user/user/get_account'
            payload = {
                "email": email,
            }
            async with aiohttp.ClientSession() as session:
        ...
        async def delete_account_button(self, event):
            ...
            url = 'http://127.0.0.1/admin/admin/delete_user'
            payload = {
                "email": email,
            }
            async with aiohttp.ClientSession() as session:
        ...
        async def edit_buttond(self, event):
            if all([self.edit_dropdown.value, self.edit_textfield.value]):
                ...
                url = 'http://127.0.0.1/user/user/edit_account'
                payload = {
                    "email": email,
